...
const getDnDContext = (active: Active, over: Over): CardMoveContext | null => {
  ...
  const activeCardList = cloneDeep(getCardList(activeColumnId)?.cards || []);
  const overCardList = cloneDeep(getCardList(overColumnId)?.cards || []);
...
export const eventDropAction = (active: Active, over: Over | null) => {
  ...
  if (active.data.current?.type === Type.COLUMN) {
    ...
    const columns = cloneDeep(store.getState().kanbanState.columnList);
  ...
  // Handle Card Drag
    ...
    const cardList = cloneDeep(getCardList(activeColumnId)?.cards || []);
...
export const eventDragMoveAction = (active: Active, over: Over) => {
  ...
  if (isActiveCard && isOverCard && activeColumnId !== overColumnId) {
    ...
    const newActiveCardList = cloneDeep(activeCardData.cards);
    const newOverCardList = cloneDeep(overCardData.cards);
  ...
  if (isActiveCard && !isOverCard) {
    ...
    if (activeColumnId === overColumnId) {
      ...
      const newCardList = cloneDeep(activeCardData.cards);
      let returnActiveCardList: ICard[] = [];
